function App(){
   ...
   //toFixed 소수점 자리 표시
    ...
    const result05 = foods.map((item, index) => (
       <li key={index}>맛있는 {item} 좋아요.</li>     // <ul>은 순서 정렬 없이 기호를 사용하여 목록 정렬 , <li> 리스트 항목 
    ));                                             // key={index}는 각 항목을 리스트에서 고유하게 식별
    console.log(result05);                          // index: 배열에서 각 항목의 위치를 나타내는 값
                                                    // 자바와 마찬가지로 배열은 0부터 시작한다(라면 : 0 , 우동 :1 ~)
    const result06 = foods.map((item, index) => {                    // {}쓰는 문장엔 return이 들어간다
        if(index % 2 === 0){
    ...
    words.map((sentence) => {                   //sentence = words 안에 단어들
        if(sentence.length <= mylength){        // length = 문자열에 포함된 문자 개수
            min_length.push(sentence);          // push()는 배열에 항목을 추가하는 방법
                                                // (상자에 물건을 넣는 것처럼 배열에 항목을 추가하는 거)
        }
        return 0 ;
    });
    ...
    const customers = [
        {firstname:'민정', lastname:'김', age:10},           //{}= 객체 
    ...
    function getCustomerOne(person, index){
        // 김 민정님, 나이 : xx살(미성년자)
        const isAdult = person.age >= 19 ? '성인' : '미성년자' ; // 삼항 연산자 사용
    ...
    const WinterItems = () => {
        ...
        const itemList = items.map((item, index)=>(                      // {}를 쓸땐 반드시 return, () 쓸땐 return 안써도됨
            <li key={index}>
