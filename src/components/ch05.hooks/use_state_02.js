    ...
    return(
        <div className="App">
            ...
            <select id="model" value={model} onChange={ChangeTest} >
                <option value="avante">아반뗴</option>
            ...
            <p>
               <span style={{color:color, fontWeight: 'bolder'}}>{color}</span> 색상의 <span style={{fontWeight: 'bolder'}}>{year}</span> 년산 <span style={{fontWeight: 'bolder'}}>{modelName}</span> 모델<br />
                <br />
                {comment}
            </p>
            <br /><br />

            <img src={`/images/${image}.png`} alt={model} width={imageSize} height={imageSize} />
        </div>
    );
}
