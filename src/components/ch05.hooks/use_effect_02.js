...
function App(){
    ...
    const TodoSomething = () => {
        ...
        if(imageIdx === (imageArray.length -1)){ // 배열 범위를 벗어나면 0으로 초기화
            setImageIdx(0);
        }
    ...
    return(
        <div className="App">
            ...
            <hr />
            {/* 화살표 함수를 직접 속성창에 작성할수 있습니다. */}
            <button onClick={() => { setCount((count) => count + 1) }}>
                &nbsp;카운트 값1 더하기&nbsp;
            </button>
            <hr />
            <div>
                <img src={image} alt="대안 이미지" width={imagSize} height={imagSize} />
            </div>
        </div>
    );
}
